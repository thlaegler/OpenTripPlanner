...
public class GrizzlyServer {
  ...
  public void run() {
    ...
    // TODO we should probably use Grizzly async processing rather than tying up the HTTP handler
    // threads.
    int nHandlerThreads = getMaxThreads();
    ThreadPoolConfig threadPoolConfig = ThreadPoolConfig.defaultConfig()
        .setCorePoolSize(nHandlerThreads).setMaxPoolSize(nHandlerThreads).setQueueLimit(-1);

    /* HTTP (non-encrypted) listener */
    NetworkListener httpListener =
        new NetworkListener("otp_insecure", params.bindAddress, params.port);
    httpListener.setSecure(false);

    /* HTTPS listener */
    NetworkListener httpsListener =
        new NetworkListener("otp_secure", params.bindAddress, params.securePort);
    ...
    // For both HTTP and HTTPS listeners: enable gzip compression, set thread pool, add listener to
    // httpServer.
    ...
    CLStaticHttpHandler staticHandler =
        new CLStaticHttpHandler(GrizzlyServer.class.getClassLoader(), "/client/");
    ...
    if (params.clientDirectory != null) {
      StaticHttpHandler localHandler =
          new StaticHttpHandler(params.clientDirectory.getAbsolutePath());
    ...
    try {
      ...
      if (profile != null && profile.equalsIgnoreCase("BUILDER")) {
        LOG.info(
            "Successfully built graph. Shuting down now since OTP has been started as 'BUILDER' only");
