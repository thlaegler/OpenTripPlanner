...
public class DirectTransferAnalyzer implements GraphBuilderModule {
  ...
  public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra,
    ...
    for (TransitStopVertex originStop : Iterables.filter(graph.getVertices(),
        TransitStopVertex.class)) {
      ...
      Map<TransitStopVertex, NearbyStopFinder.StopAtDistance> stopsEuclidean =
          nearbyStopFinderEuclidian.findNearbyStopsEuclidean(originStop).stream()
              .collect(Collectors.toMap(t -> t.tstop, t -> t, (a, b) -> b));
      ...
      List<TransitStopVertex> stopsConnected = stopsEuclidean.keySet().stream()
          .filter(t -> stopsStreets.keySet().contains(t) && t != originStop)
          .collect(Collectors.toList());

      /* Get stops found by euclidean search but not street search */
      List<TransitStopVertex> stopsUnconnected = stopsEuclidean.keySet().stream()
          .filter(t -> !stopsStreets.keySet().contains(t) && t != originStop)
          .collect(Collectors.toList());
      ...
      for (TransitStopVertex destStop : stopsConnected) {
        ...
        TransferInfo transferInfo =
            new TransferInfo(originStop, destStop, euclideanStop.distance, streetStop.distance);
      ...
      for (TransitStopVertex destStop : stopsUnconnected) {
        ...
        directTransfersNotFound
            .add(new TransferInfo(originStop, destStop, euclideanStop.distance, -1));
      }
    }
    ...
    for (TransferInfo transferInfo : directTransfersTooLong) {
      issueStore
          .add(new TransferRoutingDistanceTooLong(transferInfo.origin, transferInfo.destination,
              transferInfo.directDistance, transferInfo.streetDistance, transferInfo.ratio));
    }
    ...
    for (TransferInfo transferInfo : directTransfersNotFound) {
      issueStore.add(new TransferCouldNotBeRouted(transferInfo.origin, transferInfo.destination,
    ...
    LOG.info(
        "Done analyzing transfers. {} transfers could not be routed and {} transfers had a too long routing"
            + " distance.",
        directTransfersNotFound.size(), directTransfersTooLong.size());
  }
  ...
  private static class TransferInfo {
    ...
    TransferInfo(TransitStopVertex origin, TransitStopVertex destination, double directDistance,
