 ...
 * <p>
 * This configuration is optional an found under "feature" in the top level 'otp-config.json' file.
 */
public enum OTPFeature {
  APIExternalGeocoder(true), APIBikeRental(true), APIAlertPatcher(true), APIServerInfo(
      true), APIGraphInspectorTile(true), APIUpdaterStatus(true),

  // Sandbox extension features - Must be turned OFF by default
  ActuatorAPI(true), GoogleCloudStorage(true), SandboxAPITransmodelApi(
      true), SandboxExampleAPIGraphStatistics(true), TransferAnalyzer(false);
  ...
  /**
   * This method allowes the application to initilize each OTP feature. Only use this method at
   * startup-time.
   *
  ...
  private static String valuesAsString(boolean enabled) {
    return Arrays.stream(values()).filter(it -> it.enabled == enabled).map(Enum::name)
