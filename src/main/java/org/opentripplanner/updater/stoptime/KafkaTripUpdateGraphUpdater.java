 ...
 */
@Slf4j
public class KafkaTripUpdateGraphUpdater implements GraphUpdater {
  ...
  private String tripUpdateTopic;

  private KafkaListener tripUpdateKafkaListener;
  ...
  public void configure(Graph graph, JsonNode config) throws Exception {
    ...
    if (profile != null && profile.equalsIgnoreCase("SERVER")) {
      ...
      this.tripUpdateTopic = config.path("tripUpdateTopic").asText();
      tripUpdateKafkaListener = createListener();
  ...
  private KafkaListener createListener() {
    return new KafkaListener(url, tripUpdateTopic) {
      @Override
      void handleFeedMessage(FeedEntity feed) {
        handleFeedEntity(feed);
      }
