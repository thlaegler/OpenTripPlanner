...
public class CommandLineParameters implements Cloneable {
  ...
  @Parameter(names = {"--save"}, description = "Save the 'graph.obj' to local disk or data source "
  ...
  @Parameter(names = {"--disableFileCache"},
  ...
  @Parameter(names = {"--baseDirectory"}, validateWith = ReadWriteDirectory.class,
      description = "/graph/or/inputs/directory")
  ...
  private void validateParameterCombinations() {
    List<String> cmds = listParams(List.of("--load", "--build", "--loadStreet", "--buildStreet"),
