...
public class OTPMain {
  ...
  /**
   * Parse and validate command line parameters. If the arguments is invalid the method uses
   * {@code System.exit()} to exit the application.
   */
  private static CommandLineParameters parseAndValidateCmdLine(String[] args) {
    CommandLineParameters params = new CommandLineParameters();
    try {
      // It is tempting to use JCommander's command syntax:
      // http://jcommander.org/#_more_complex_syntaxes_commands
      // But this seems to lead to confusing switch ordering and more difficult subsequent use of
      // the
  ...
  private static void startOTPServer(CommandLineParameters params) {
    ...
    if (params.doLoadGraph() || params.doLoadStreetGraph()) {
      DataSource inputGraph =
          params.doLoadGraph() ? app.store().getGraph() : app.store().getStreetGraph();
    ...
    // This would also avoid the awkward call to set the router on the appConstruction after it's
    // constructed.
    // However, currently the server runs in a blocking way and waits for shutdown, so has to run
    // last.
    if (params.doServe()) {
      ...
      while (true) {
        try {
          ...
          LOG.error("An uncaught error occurred inside OTP. Restarting server. Error was: {}",
