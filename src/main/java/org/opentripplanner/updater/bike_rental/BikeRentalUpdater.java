...
public class BikeRentalUpdater extends PollingGraphUpdater {
  ...
  Map<BikeRentalStation, BikeRentalStationVertex> verticesByStation =
      new HashMap<BikeRentalStation, BikeRentalStationVertex>();
  ...
  protected void configurePolling(Graph graph, JsonNode config) throws Exception {
    ...
    // bikes at
    // stations for another network. TODO shouldn't we give each updater a unique network ID by
    // default?
    ...
    if (sourceType != null) {
      if (sourceType.equals("jcdecaux")) {
        ...
        source = new NextBikeJsonRentalDataSource(networkName);
      ...
      } else if (sourceType.equals("onzo")) {
        source = new OnzoJsonBikeRentalDataSource(networkName);
      } else if (sourceType.equals("lime")) {
        source = new LimeScooterJsonBikeRentalDataSource(networkName);
      }
    }
    ...
    if (pollingPeriodSeconds <= 0) {
      ...
      LOG.info("Creating bike-rental updater running every {} seconds: {}", pollingPeriodSeconds,
          source);
    }
